...
class App {
	...
	private makeTextPlane(text: string, color: string, size: number) {
		...
		const plane = BABYLON.MeshBuilder.CreatePlane('textPlain', { size, updatable: true }, this.scene);
		const mat = new BABYLON.StandardMaterial('textPlaneMaterial', this.scene);
