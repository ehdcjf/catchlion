...
class App {
	...
	setLight() {
		const hemiLight = new HemisphericLight('hlight', new Vector3(0, 1, 0), this.scene);
		hemiLight.intensity = 0.3;

		const plight = new PointLight('plight', new Vector3(3, 3, -3), this.scene);
	...
	buildTile() {
		...
		for (let index = 0; index < this.gridWidth * this.gridWidth; index++) {
			...
			tile.id = `${index}`;
			tile.position = new Vector3(Math.floor(index / this.gridWidth), 0.55, index % this.gridWidth);
		}
		originTile.dispose();
	}
	...
	async loadChessModelAsync() {
		...
		originChick.addRotation(Tools.ToRadians(90), Tools.ToRadians(180), Tools.ToRadians(90));
	...
	rotate(gridPosition: number, count: number, isClockwise = true) {
		...
		for (let index = 0; index < count; index++) {
			...
			if (isClockwise) {
				gridPosition = x * this.gridWidth + (this.gridWidth - 1 - y);
			} else {
				gridPosition = (this.gridWidth - 1 - x) * this.gridWidth + y;
			}
