...
class App {
	...
	private async _goToCutScene(): Promise<void> {
		...
		this._cutScene.onBeforeRenderObservable.add(() => {
			...
			if (this.finishedLoading && canplay) {
				canplay = false;
				this._goToGame();
			}
		});
	...
	private async _goToGame(): Promise<void> {
		...
		this._ui.setStartTimer();
		this._ui.startSparkTimer(this._player.sparkler);
	...
	private async _initializeGameAsync(scene): Promise<void> {
		...
		scene.onBeforeRenderObservable.add(() => {
			...
			if (this._player.sparkReset) {
				...
				this._ui.updateLanternCount(this._player.lanternLit);
			}
			...
			if (this._player.win && this._player.lanternLit == 22) {
			...
			if (
				this._player.tutorialMove &&
				this._player.tutorialJump &&
				this._player.tutorialDash &&
				(this._ui.tutorial.isVisible || this._ui.hint.isVisible)
			) {
				this._ui.tutorial.isVisible = false;
				if (!this._environment.lanternArray[1].isLit) {
		...
		gl.intensity = 0.4;
		this._environment.lanternArray.forEach((lantern) => {
